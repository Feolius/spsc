...
# state = spsc_core.StateSimple.import_file('data/solutions/x_electrons_superlattice/C237.yml')

state = spsc_data_generator.x_electrons_superlattice_diff_mass('data/data_generator/x_electrons_superlattice_diff_mass/C237.yml')
solver = spsc_solver.LatticeXElectronsSymmetrySolver(state)
solver.solve()
state = solver.state
state.export_file('data/solutions/x_electrons_superlattice/C237.yml')
...
for tick in ax.yaxis.get_major_ticks():
    tick.label.set_fontsize(16)
plt.savefig("C237_solution.png", dpi=400, bbox_inches='tight')
plt.show()
